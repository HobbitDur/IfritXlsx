...
class IfritXlsxWidget(QWidget):
    ...
    def __init__(self, icon_path=os.path.join(os.path.dirname(os.path.realpath(__file__)),"Resources")):
        ...
        #self.dat_file_selected = ['I:/Mod_FF8/Outils modding/Fichier de travail/GitProject/IfritEnhanced/IfritXlsx/OriginalFiles/c0m001.dat']
            ...
            if self.process_selector.currentIndex() == 0:  # Dat to xlsx
                if self.limit_option.value() != -1:
                    ...
                    for current_path in self.dat_file_selected:
                        if int(pathlib.Path(current_path).name.split('.')[0].replace('c0m', '')) == self.limit_option.value():
                            dat_file_current_list = [current_path]
                    if not dat_file_current_list:
                        text_error = "Monster ID {} not found in .dat file loaded, please either change loaded files or the monster ID".format(self.limit_option.value() )
                        message_box = QMessageBox()
                        message_box.setText(text_error)
                        message_box.setIcon(QMessageBox.Icon.Critical)
                        message_box.setWindowIcon(self.__ifrit_icon)
                ...
                self.ifrit_manager.xlsx_to_dat(self.xlsx_file_selected, self.limit_option.value(), self.analyse_ai.isChecked())
